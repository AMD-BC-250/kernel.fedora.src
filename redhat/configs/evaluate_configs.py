...
def main() -> None:
    ...
    try:
        ...
        for i, config_name in enumerate(configs_to_process, 1):
            if args.debug == 0:
                print(f"\r\033[K({i:0{width}d}/{len(configs_to_process)}) Analyzing {config_name}", end='', flush=True)
            else:
                reader.msg(f"\n{'='*80}")
                reader.msg(f"{i:0{width}d}/{len(configs_to_process)} Analyzing {config_name}")
                reader.msg(f"{'='*80}")
